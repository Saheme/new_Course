...
//import static edu.javaRushCourse.JavaSyntax.level7.lesson4.Task130.Mrakobes.setIsAlive;

/**

 * Для того, чтобы получить доступ к приватной переменной isAlive класса Mrakobes, создай в этом классе публичный статический метод void setIsAlive(boolean),
 * который будет принимать на вход значение true или false и присваивать это значение переменной isAlive класса Mrakobes.
 * Вызови этот метод с нужным значением в методе main вместо обращения к приватной переменной. Также исправь ошибку с вызовом метода printState.
 *
 * Подсказка: тут тоже не обошлось без модификатора доступа.
 *
 * Требования:
 * •	В классе Mrakobes должна быть приватная статическая переменная isAlive типа boolean, проинициализированная при объявлении значением true.
